...
from DataSynthesizer.lib.PrivBayes import greedy_bayes, greedy_bayes_new, greedy_bayes_new2, construct_noisy_conditional_distributions, construct_noisy_conditional_distributionsmod
...
class DataDescriber:
    ...
    def describe_dataset_in_correlated_attribute_mode(self,
        ...
        if k > 0:
            self.bayesian_network, root_attribute = greedy_bayes(self.df_encoded, k, epsilon * beta, seed=seed)
            self.data_description['conditional_probabilities'] = construct_noisy_conditional_distributions(
            self.bayesian_network, self.df_encoded, root_attribute, epsilon * (1-beta))
            #self.bayesian_network, root_attribute = greedy_bayes(self.df_encoded, k, epsilon * beta, beta, theta, seed=seed)
        else:
            self.bayesian_network, root_attribute = greedy_bayes_new(self.df_encoded, k, epsilon * beta, beta, theta, seed=seed)
            self.data_description['conditional_probabilities'] = construct_noisy_conditional_distributionsmod(
            self.bayesian_network, self.df_encoded, root_attribute, epsilon * (1-beta))

        self.data_description['bayesian_network'] = self.bayesian_network
        self.data_description['root'] = root_attribute
